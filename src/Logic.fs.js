...
export function render() {
    ...
    if (!Operators_IsNull(Operators_IsNull(display) ? document.querySelector(".lcd-screen") : display)) {
        throw 1;
    }
        ...
        switch (msg) {
            ...
            case "DEL":
                if (state.Display === "ERROR") {
                    ...
                    return new State((state.Display.length <= 1) ? "0" : substring(state.Display, 0, state.Display.length - 1), state.IsExploded, state.IsArming);
                }
            default:
                if ((digit = msg, (digit.length === 1) && isDigit(digit[0]))) {
                    const digit_1 = msg;
                    const newDisplay = ((state.Display === "0") ? true : (state.Display === "ERROR")) ? digit_1 : (state.Display + digit_1);
